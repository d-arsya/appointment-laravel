<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Ubah jadwal janji temu anda') }}
        </p>
    </header>

    <form method="post" action="{{ route('appointment.update', $appointment->id) }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <x-dropdown-input id="doctor_id" name="doctor_id" type="text" class="mt-1 block w-full" required autofocus
                ...
                @foreach ($doctors as $item)
                    <option {{ $appointment->doctor_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                        {{ $item->name }}</option>
                @endforeach
            </x-dropdown-input>
        ...
        <div>
            ...
            <x-text-input id="time" name="time" :value="old('time', \Carbon\Carbon::parse($appointment->book)->isoFormat('h'))" type="number" class="mt-1 block w-full"
        ...
        <div>
            ...
            <x-text-input id="date" name="date" type="date" :value="old('time', \Carbon\Carbon::parse($appointment->book)->format('Y-m-d'))" class="mt-1 block w-full"
